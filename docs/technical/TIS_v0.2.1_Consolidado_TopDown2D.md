...
# CONSOLIDAÇÃO DE IMPLEMENTAÇÃO PÓS-MVP — 2026-08-24

> **Decisão aprovada:** implementação oficial em **Top-Down 2D** usando Godot 4.7.1 / GDScript 2.0.
>
> Esta versão mantém a especificação técnica-base e acrescenta o baseline pós-MVP, regras de integração visual e observações de decisões ainda pendentes.
>
> **Regra:** o TIS define COMO implementar. Ele não deve criar regras de gameplay que não tenham sido aprovadas no GDD/TDD/BALANCE.

## 0.A Baseline Real

Funcionalidades registradas como concluídas na governança:

- Player 2D e MovementComponent2D;
- câmera 2D;
- Survival;
- Interaction 2D;
- Inventory/Items;
- Gathering/Mina de Ferro;
- Crafting/Profession;
- Lighting 2D;
- Combat/Enemy;
- NPC/Quest;
...
- Save/Load;
- testes associados às funcionalidades registradas.

## 0.B Regras para a Playable Build

A camada visual deverá ser construída em cima dos contratos existentes.
...
- fornecer feedback;
- organizar UI;
- representar mundo, NPCs, itens e recursos;
- suportar testes manuais.

### Não deve fazer

- alterar regras de domínio diretamente;
- duplicar cálculos;
- criar uma segunda economia;
- criar uma segunda implementação de inventário;
- criar uma segunda lógica de iluminação;
- acessar estado interno de serviços sem contrato.

## 0.C Fluxo de Apresentação

```text
Domain State
    ↓
Event / Query / ViewModel
    ↓
Presentation
    ↓
HUD / Inventory / Equipment / World Visual
```

## 0.D Multiplayer Playable Build

Antes de declarar a Playable Build multiplayer como validada, testar:

1. conexão;
2. spawn;
3. identificação por EntityId;
4. movimento;
5. interação;
6. inventário;
7. itens;
...
`CharacterBody2D`, `CharacterController`, `MovementComponent`, `CameraController`.
...
# 10. Decisões de Implementação a Tomar / Confirmar

# 8.5. Baseline Técnico Oficial Pós-Decisões

**Multiplayer:** `ENetMultiplayerPeer` + `NetworkTransportService` + Listen Server / Host Autoritativo.

**Navegação:** `NavigationServer2D` + `NavigationRegion2D` + `NavigationAgent2D`, acessados pela IA somente através de `NavigationService`; obstáculos dinâmicos via `NavigationObstacle2D`.

**World Streaming:** Grid-Based Chunks 2D, com Full Simulation em regiões ativas e Offline/Abstract Simulation em regiões inativas.

**Apresentação:** MVVM-like / Presentation Model, UIManager, HUD/Window/Overlay Layers, inventário visual e minimapa.

**Direção visual:** Pixel Art Neons/Chiaroscuro, `640 × 360`, tiles `16 × 16`, Player `32 × 32`.

**Regra:** qualquer mudança dessas decisões exige nova decisão formal e sincronização de GDD, TDD, TIS e governança.

## DEC-TIS-001 — Estrutura Final de Cenas 2D

**Estado:** APROVADA.

Contratos oficiais: `Player.tscn` usa `CharacterBody2D` com visual, colisão, `MovementComponent2D`, `SurvivalComponent`, `CombatComponent`, `InteractionDetector` e `PointLight2D`; NPC/Enemy usam `CharacterBody2D`, `AIController`, `NavigationAgent2D` e componentes de estado; ResourceNode usa `StaticBody2D`; Region usa `Node2D`, `TileMapLayer`, `NavigationRegion2D`, containers de entidades/luzes e áudio.

Composição é obrigatória; regras de domínio não ficam nos Nodes visuais.

## DEC-TIS-002 — Padrão de UI

**Estado:** APROVADA.

MVVM-like / Presentation Model com `UIManager.tscn` como `CanvasLayer`, dividido em `HUDLayer`, `WindowLayer` e `OverlayLayer`. ViewModels intermediam o domínio. Theme Resource global e atualização reativa via `EventBus`. Resolução-base `640 × 360` com escalonamento proporcional e Pixel Snap.

## DEC-TIS-003 — Inventário Visual

**Estado:** APROVADA.

`GridContainer` com slots dinâmicos; drag-and-drop via métodos nativos; `SwapSlotsCommand`/`MoveItemCommand`; Shift+clique para split; botão direito para consumo/equipamento; painel de equipamento e tooltip contextual. `InventoryService` é a única fonte de verdade.

## DEC-TIS-004 — Minimap

**Estado:** APROVADA.

`SubViewport` + `Camera2D` ortográfica dedicada no canto superior direito. `MinimapService` gerencia marcadores de jogador, coop, NPCs, recursos, inimigos conforme percepção/luz e objetivos de quest. Fog of War usa estado de exploração/iluminação sem criar fonte de verdade paralela.

## DEC-TIS-005 — Assets

**Estado:** APROVADA.

Pixel Art Neons/Chiaroscuro. Resolução `640 × 360`; tiles `16 × 16`; Player `32 × 32`; Pixel Snap; Filter `Nearest`; Mipmaps desativados para pixel art. PNG para sprites, OGG para música/áudios longos e WAV para SFX curtos. Estrutura `res://assets/{textures,audio,fonts}` conforme categorias existentes e nomenclatura `snake_case`.

## DEC-TIS-006 — Local Jogável Inicial

**Estado:** APROVADA.

**Posto Avançado da Garganta de Ferro:** fronteira Terras Alinhadas/Setor Escuro, Farol do Alinhamento, NPC Guardião/Mercador com diálogo/loja, Forja, caverna com 2 nós de Minério de Ferro, 1 Flora Bioluminescente e spawner com 2 inimigos sensíveis à luz.

Loop: `Coletar → Forjar → Enfrentar Inimigo → Negociar → Salvar`.

## DEC-TIS-007 — Network Transport

**Estado:** APROVADA.

`ENetMultiplayerPeer` nativo do Godot 4.7.1 sobre UDP. `NetworkTransportService` encapsula o peer e permite adaptadores futuros sem alterar o domínio.

## DEC-TIS-008 — Sincronização de Estado

**Estado:** APROVADA.

| Categoria | Dados | Método |
|---|---|---|
| Autoritativos | Economia, XP, Relacionamentos, Quests, Atributos | Servidor altera; RPC/eventos validados |
| Replicados | Posição de jogadores/NPCs, animações, luzes ativas | `MultiplayerSynchronizer` a 15–20 Hz + interpolação |
| Preditivos | Movimento imediato e ataques locais | Execução local + validação por Command |
| Locais | UI, partículas, VFX secundários | EventBus local |
| Persistentes | EntityId, Inventário, Durabilidade, Estado da Vila | Save transacional/World Tick |
| Transitórios | Projéteis, stagger, partículas, marcadores temporários | RPC `unreliable`, sem save |

## DEC-TIS-009 — Testes Manuais

**Estado:** DEFINIDA / EXECUÇÃO PENDENTE.

Campos obrigatórios: **ID | Categoria | Pré-condição | Passos | Resultado Esperado | Resultado Obtido | Evidência | Severidade | Status**.

Testes mínimos definidos:
- `TM-001`: Core Loop & Save.
- `TM-002`: Luz & Combate.
- `TM-003`: Coop / Host & Client.

Os testes estão especificados, mas ainda não foram executados/validados.

## DEC-TIS-010 — Critérios da Playable Build

**Estado:** DEFINIDA / VALIDAÇÃO PENDENTE.

Os critérios estão definidos, porém **não foram considerados executados**.

- [ ] Interface legível.
- [ ] Inventário visual operacional.
- [ ] Minimap ativo.
- [ ] Loop de ação concluído.
- [ ] Multiplayer cooperativo básico via ENet.
- [ ] Save/Load íntegro.
- [ ] Zero erros críticos nos logs.

A Playable Build só poderá ser declarada pronta após execução, evidência e aprovação de 100% dos critérios.
